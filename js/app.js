...
async function downloadFullApp() {
    ...
    try {
        ...
        const fetchBlobFile = async (path) => {
            const response = await fetch(path);
            if (!response.ok) {
                throw new Error(`Failed to fetch ${path} (${response.status})`);
            }
        ...
        // Add HTML file
        const htmlContent = await fetchTextFile('index.html');
        zip.file('index.html', htmlContent);
        
        // Add install.html
        const installContent = await fetchTextFile('install.html');
        zip.file('install.html', installContent);
        
        // Add CSS files
        const cssFolder = zip.folder('css');
        const cssContent = await fetchTextFile('css/style.css');
        cssFolder.file('style.css', cssContent);
        
        // Add JS files
        const jsFolder = zip.folder('js');
        const jsContent = await fetchTextFile('js/app.js');
        jsFolder.file('app.js', jsContent);
        
        // Add icon
        const iconBlob = await fetchBlobFile('icon-512.png');
        zip.file('icon-512.png', iconBlob);
        
        // Add manifest.json
        const manifestContent = await fetchTextFile('manifest.json');
        zip.file('manifest.json', manifestContent);
        
        // Add service worker
        const swContent = await fetchTextFile('sw.js');
        zip.file('sw.js', swContent);
        
        // Add documentation files
        const readmeContent = await fetchTextFile('README.md');
        zip.file('README.md', readmeContent);
        
        const changelogContent = await fetchTextFile('CHANGELOG.md');
        zip.file('CHANGELOG.md', changelogContent);
        
        const licenseContent = await fetchTextFile('LICENSE.md');
        zip.file('LICENSE.md', licenseContent);
        
        // Add PWA installation guide
        try {
            const pwaGuideContent = await fetchTextFile('PWA-INSTALL-GUIDE.md');
