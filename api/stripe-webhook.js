...
async function insertPaymentIfNotExists(paymentPayload) {
    const transactionId = paymentPayload.transaction_id;
    ...
    if (existingPayment) {
        console.log(`Payment already exists for user ${userId}, transaction ${transactionId}`);
        return;
    }
...
async function handleCheckoutCompleted(session) {
    ...
    await insertPaymentIfNotExists({
        user_id: userId,
...
async function handleSubscriptionUpdate(subscription) {
    ...
    if (!userId) {
        ...
        if (profileError) {
            console.error('Failed to find user by customer ID:', profileError);
            throw profileError;
        }
...
async function handleSubscriptionDeleted(subscription) {
    ...
    if (profileError) {
        console.error('Failed to find profile for deleted subscription:', profileError);
        throw profileError;
    }
...
async function handleInvoicePaymentSucceeded(invoice) {
    ...
    if (profileError) {
        console.error('Failed to find profile for paid invoice:', profileError);
        throw profileError;
    }
...
async function handleInvoicePaymentFailed(invoice) {
    ...
    if (profileError) {
        console.error('Failed to find profile for failed invoice:', profileError);
        throw profileError;
    }
    ...
    if (statusError) {
        console.error('Failed to update profile to past_due:', statusError);
        throw statusError;
    }
